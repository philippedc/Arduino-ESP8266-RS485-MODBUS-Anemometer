...
// store the max value
  if( Anemometer > max_Anemometer ) {
    ...
    max_wind_date = String(daysOfTheWeek[weekday()-1])+" "+ String(day())+"/"+String(month())+" &agrave; "+String(hour())+Minutes; 
  }
...
void webserver() { 
  ...
  if( client ) {                            // If a new client connects,
    ...
    // Web Page Heading
    client.println("<body><h1>An&eacute;mom&egrave;tre chez Fifi</h1>");
    String Minutes = "h";
    if( minute() < 10 ) Minutes += "0"+String(minute());
    else Minutes += String(minute());
    ...
    client.println("<HR size=2 align=center width=\"80%\">");
    client.println("<p><h3>Maxi mesur&eacute; " + String(max_Anemometer) + " m/s");
